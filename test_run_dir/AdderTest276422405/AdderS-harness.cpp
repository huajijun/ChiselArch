...
#endif
#include <iostream>
class AdderS_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    AdderS_api_t(VAdderS* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_b)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_a)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_out)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["AdderS.reset"] = 0;
    }
#if VM_TRACE
...
int main(int argc, char **argv, char **env) {
    ...
    VAdderS* top = new VAdderS;
    std::string vcdfile = "test_run_dir/AdderTest276422405/AdderS.vcd";
